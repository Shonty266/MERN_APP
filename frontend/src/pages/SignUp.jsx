...
const SignUp = () => {
    ...
    const handleSignup = async (e) => {
        ...
        setIsLoading(true)
        try {
            const url = `${BASE_URL}/auth/admin/signup`
        ...
        <div className="w-full h-screen flex overflow-hidden">
            <div className="lg:w-1/2 w-full lg:h-full h-fit py-20 lg:px-10 px-4 relative flex flex-col justify-between">
                <div className="logo-div flex justify-between items-center lg:px-10 px-4">
                    ...
                    <a href="/admin/login"><div className="flex items-center group"><h1>Already a member? <span className="text-bold hover:font-bold cursor-pointer transition-all duration-300 inline-flex items-center gap-1 group-hover:translate-x-1">Login<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="w-4 h-4 opacity-0 group-hover:opacity-100 group-hover:[stroke-width:3] transition-all duration-500"><path strokeLinecap="round" strokeLinejoin="round" d="M8.25 4.5l7.5 7.5-7.5 7.5" /></svg></span></h1></div></a>
                </div>
                
                <div className="signup-div lg:px-20 px-4 mt-20">
                    <div>
                        <h1 className="text-4xl font-bold">Sign Up</h1>
                    </div>
                    <form onSubmit={handleSignup} className="mt-10">
                        <div>
                            <h3 className="text-md mb-1 font-semibold">Name</h3>
                            <input
                                type="text"
                                name="name"
                    ...
                    <div className="flex justify-center items-center gap-10 lg:flex-nowrap flex-wrap">
                        <button type="button" className="lg:w-1/2 w-full py-4 border border-gray-300 rounded-md flex items-center justify-center gap-2">
            ...
            {toasts.map(toast => (
                <Toast key={toast.id} message={toast.message} visible={true} />
            ))}
        </div>
    )
}
